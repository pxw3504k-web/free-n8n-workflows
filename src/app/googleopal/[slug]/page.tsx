...
import { getOpalApp, getAllOpalApps, OpalApp } from '@/lib/data';
...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  const ogImageUrl = `${siteUrl.replace(/\/$/, '')}/api/og/googleopal?title=${encodeURIComponent(appTitle)}&icon=${iconParam}`;
  ...
  return {
    ...
    openGraph: {
      ...
      description: seoDescription,
      type: 'website',
      url: `${siteUrl.replace(/\/$/, '')}/googleopal/${slug}`,
    ...
    alternates: {
      canonical: `${siteUrl.replace(/\/$/, '')}/googleopal/${slug}`,
      languages: {
        'en-US': `${siteUrl.replace(/\/$/, '')}/googleopal/${slug}`,
        'zh-CN': `${siteUrl.replace(/\/$/, '')}/googleopal/${slug}?lang=zh`,
      },
    },
    robots: {
      index: true,
...
export default async function OpalAppPage({ params, searchParams }: PageProps) {
  ...
  if (app?.related_n8n_workflow_ids && app.related_n8n_workflow_ids.length > 0) {
    try {
      console.log('Fetching related workflows for app:', slug, 'IDs:', app.related_n8n_workflow_ids);
      
      const { data, error } = await supabase
        .from('workflows')
        .select('id, title, title_zh, slug, summary_short, summary_short_zh, stats, category, category_zh, tags, tags_zh, json_url')
        .in('id', app.related_n8n_workflow_ids)
        .limit(3); // 限制显示 3 个
      
      if (error) {
        console.error('Error fetching related workflows:', {
        ...
        if (data && data.length > 0) {
          console.log('Workflow IDs found:', data.map(w => w.id));
        }
      }
      
      // 将查询结果转换为 WorkflowData 格式，添加必要的默认值
      if (data && data.length > 0) {
        relatedWorkflows = data.map((workflow) => ({
          id: workflow.id,
          slug: workflow.slug,
          title: workflow.title || '',
          title_zh: workflow.title_zh || undefined,
          summary_short: workflow.summary_short || '',
          summary_short_zh: workflow.summary_short_zh || undefined,
          image: undefined, // workflows 表没有 icon/image 字段
          stats: workflow.stats || {},
          category: workflow.category || '',
          category_zh: workflow.category_zh || undefined,
          tags: Array.isArray(workflow.tags) ? workflow.tags : [],
          tags_zh: Array.isArray(workflow.tags_zh) ? workflow.tags_zh : undefined,
          json_url: workflow.json_url || undefined,
          created_at: new Date().toISOString(), // 默认值
        })) as WorkflowData[];
      }
    } catch (err) {
      console.error('Exception fetching related workflows:', err);
    }
  } else {
    console.log('No related_n8n_workflow_ids for app:', slug, 'app.related_n8n_workflow_ids:', app?.related_n8n_workflow_ids);
  }
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              {/* Pro Version Upsell Card - Always Show */}
              {relatedWorkflows.length > 0 ? (
                <div className="rounded-2xl border-2 border-indigo-500/30 bg-indigo-500/5 backdrop-blur-sm p-8 shadow-xl relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    {/* AIWord Cloud Link - Below Workflow List */}
                    <div className="pt-4 border-t border-indigo-500/20">
                      <a
                        href="https://aiword.life/automations?source=opal_upsell"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 text-sm font-medium text-indigo-300 hover:text-indigo-200 transition-colors group"
                      >
                        {lang === 'zh' 
                          ? '或者，在 AIWord Cloud 上完全托管运行 →'
                          : 'Or run fully managed on AIWord Cloud →'}
                ...
                /* Fallback: AIWord Cloud Card when no related workflows */
                <div className="rounded-2xl border-2 border-violet-500/30 bg-gradient-to-br from-violet-500/10 to-indigo-500/10 backdrop-blur-sm p-8 shadow-xl relative overflow-hidden">
                  {/* 背景装饰 */}
                  <div className="absolute top-0 right-0 w-64 h-64 bg-violet-500/10 blur-3xl rounded-full -z-10" />
                  
                  <div className="relative z-10">
                    <div className="flex items-start gap-6 mb-6">
                      <div className="flex-shrink-0">
                        <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center">
                          <Zap className="w-8 h-8 text-white" />
                        </div>
                      </div>
                      <div className="flex-1">
                        <h2 className="text-2xl font-bold text-white mb-3">
                          {lang === 'zh' 
                            ? '🚀 使用 AIWord Cloud 自动化'
                            : '🚀 Automate this with AIWord Cloud'}
                        </h2>
                        <p className="text-gray-300 leading-relaxed mb-6">
                          {lang === 'zh'
                            ? 'Opal 生成想法，AIWord 运行业务。将 AI 连接到真实世界的数据。'
                            : 'Opal generates the idea. AIWord runs the business. Connect AI to real-world data.'}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* CTA Button */}
                    <a
                      href="https://aiword.life/automations?source=opal_fallback"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white font-semibold rounded-lg transition-all duration-200 shadow-lg shadow-violet-500/25 hover:shadow-violet-500/40"
                    >
                      {lang === 'zh' 
                        ? '探索 AI 自动化 ↗'
